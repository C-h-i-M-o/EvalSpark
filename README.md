...
V3 RAG 正在按 [需求规格与分阶段实施计划](docs/v3-rag-spec-plan.md) 开发。已有私有检索基础设施、知识库/文档管理 API、四格式解析、Qwen Token 切分、异步索引与清理。当前设备内存不足，按用户决定优先交付代码，真实模型全链路和本地部署留待资源充足设备验收；已执行与延期项见计划。RAG 评测和 React 页面尚未开放。迁移 `20260902_01/02` 仅在隔离测试库验证，业务升级需先确认备份与恢复方案。Agent 工具评测与 AI 安全测试集将在 RAG 之后单独设计。

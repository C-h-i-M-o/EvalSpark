...
V3 RAG 正在按 [需求规格与分阶段实施计划](docs/v3-rag-spec-plan.md) 开发。已有私有知识库/文档 API、四格式索引、逐模型检索回答与证据快照；阶段 5 已接入三轮忠实度/引用联合评分、任务 API、反馈重算和中断恢复代码，React 页面待阶段 6。当前设备内存不足且 Docker 已关闭，优先交付代码：阶段 5 新增测试、真实模型全链路和本地部署均待资源充足设备验收，不能视为通过。迁移 `20260902_01/02` 仅在隔离测试库验证，业务升级需先确认备份与恢复方案。Agent 工具评测与 AI 安全测试集将在 RAG 之后单独设计。

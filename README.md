...
当前开发代码包含未部署到业务库的迁移 `20260918_01`、`20260920_01`、`20260922_01`；启动本轮多轮升级前需按规格计划备份并另行授权业务迁移，勿直接在旧业务库运行新版代码。

普通/RAG 多轮长对话升级正在开发，普通/RAG 生成 HTTP、历史引用与新证据评分已有隔离验证，双工作台、历史入口和部分会话报告已接入代码；报告跨窗口联合判断及工作台联合验收尚未完成。范围、实现状态和验收记录见 [多轮规格与计划](docs/multiturn-spec-plan.md)。

2026-09-22 本设备 Docker 暂不可用，按授权仅检查代码并提交 GitHub，本轮新增部分未运行测试或构建。新增代码包括问题后续状态展示、有界单轮评分、分支歧义依据校验、远距离窗口对及精确锚点去重，以及最新失败分支的显式重试/跳过。重试保留旧尝试费用，生成代次隔离迟到回调；超预算保持未知，不保证任意长度都能形成完整总分。代码提交不代表部署和运行验收完成。
...
本设备 Docker 当前暂不可用。其他具备 Docker 的环境可使用隔离验收入口 unit、api（不加载模型）和 integration（真实 TEI）；使用独立测试项目，不读取业务 .env。历史已运行结果和未覆盖范围见 [RAG 合并计划](docs/v3-rag-spec-plan.md)。

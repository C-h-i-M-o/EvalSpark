...
V3 RAG 正在按 [需求规格与分阶段实施计划](docs/v3-rag-spec-plan.md) 开发。当前已有私有 Embedding、向量检索和任务队列基础设施，以及私有知识库/文档管理 API；异步解析索引、RAG 评测和 React 页面尚未开放。阶段 2 增量迁移仅在隔离测试库验证，业务升级需先确认备份与恢复方案。Agent 工具评测与 AI 安全测试集将在 RAG 之后单独设计。

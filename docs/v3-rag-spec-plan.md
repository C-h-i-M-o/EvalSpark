...
> 2026-09-18：普通/RAG 同步多轮长对话升级单独在 `docs/multiturn-spec-plan.md` 维护。当前仅基础内核和会话元数据完成，RAG 多轮检索/回答尚未接入；以下单轮 V3 历史记录仍保留，不代表多轮验收结果。

## 2026-09-09 恢复开发与 Embedding 接入调整

本节是恢复开发后的最新约定，优先于下文历史阶段中“Docker 关闭、延期全部测试、固定本地 TEI”的描述。老大已启动 Docker 并要求继续 V3 开发；历史未执行记录保留，不追溯标记为通过。

### 需求与方案

- Embedding 由系统管理员在管理界面维护，不能仅依赖 `.env`。复用现有模型管理的列表、表单、密钥留空保留、启停与连接测试交互；Embedding 不进入候选回答或 Judge 模型列表。
- 统一采用 OpenAI-compatible `POST {baseUrl}/embeddings`，配置名称、Base URL、API Key、模型名、向量维度、超时、批量上限、输入长度限制及可选查询前缀。本地部署与云端使用同一协议；本地无鉴权服务允许空密钥。Base URL 包含供应商要求的版本前缀，不猜测或重复追加 `/v1`。
- 老大确认全系统单一配置，知识库不选择供应商。普通用户仅可读取模型、维度、切分单位和状态，不返回地址或密钥；升级不自动向云端传输资料。
- 单例配置带版本号防并发覆盖；更换模型、地址、维度、查询前缀等索引语义参数时，必须没有活动索引或 RAG 任务，并把所有有文档的知识库置为 `reindex_required`、递增内容版本。仅轮换密钥与超时不要求重建。
- 向量集合按配置隔离并校验维度，不能把不同模型即使同维度的向量混合检索。删除、重建、旧任务晚写继续遵循知识库锁、版本和租约约束；删除文档时也清理历史集合中的该文档向量；不提供多配置创建或删除。
- 云端路径不依赖本地 Qwen 权重或分词缓存。切块采用开源切分器的字符计数方式，明确显示计数单位；不能把字符数当作供应商 Token 或费用。旧本地分词路径保留兼容，实际 Embedding 用量以响应 `usage` 为准，缺失记未知。
- API Key 使用服务端已有凭据存储机制，列表只返回是否配置；连接测试使用固定合成短句。错误不回显上游原文、请求内容或凭据。RAG 页面说明文档块会发送到管理员配置的 Embedding 服务。
- 内置 TEI 变为显式选择的 Compose profile；普通启动与轻量验收不自动下载或加载权重。外部 Embedding、本地独立部署和可选内置 TEI 均有连接示例。

方案取舍：采用管理员独立 Embedding 配置，避免混入现有对话模型温度、最大输出等不适用参数；按确认使用全局单例，切换索引语义配置使现有知识库待重建。不增加自动模型推荐或供应商专用 SDK。

### 接口、数据与实施顺序

1. **恢复验证基线**：运行 `verify-rag.ps1 -Mode unit`，定位并修复延期期间的真实失败；补跑 React Vitest 与 TypeScript/Vite。仅使用隔离测试镜像和合成数据，不启动业务迁移。验收：记录实际 passed/skipped 与构建结果。
2. **管理员配置与协议客户端**：新增 Embedding 配置表、Alembic 迁移、类型化 schema/service/router；提供 `/api/admin/embedding-config` GET/PUT 单例读取/保存、`/test` POST 连接测试，以及登录用户 `/api/embedding-config` GET 脱敏状态。新增客户端适配 `/embeddings`，按响应 index 还原顺序，拒绝缺失/重复 index、维度错误、非有限数值、零向量。验收：RBAC、密钥脱敏/保留/轮换、状态码/超时、批次和响应校验测试。
3. **知识库与运行链路**：新增全局单例配置版本、切分单位与快照必要字段；更新 `knowledge_base_service.py`、`rag/clients.py`、`documents.py`、`indexing.py`、`evaluation_store.py`、`evaluation.py`、`usage.py` 和 Worker，按任务解析全局配置快照，确保并发请求不共享可变模型配置。验收：云端路径没有 Qwen 缓存也可索引/检索、配置切换必须重建、并发/越权/删除隔离、未知用量不显示为已知零费用。
4. **React 管理与知识库交互**：新增管理员 Embedding 管理入口；复用现有 Ant Design 表单、确认和请求错误处理。事件与异步行为放 `.ts`，`.tsx` 仅组装 UI。知识库显示计数单位和重建状态。验收：配置请求契约、管理员路由、切换过期响应保护、Vitest/构建和浏览器操作。
5. **低资源联合验收与文档**：测试 Compose 添加轻量确定性 Embedding HTTP 服务，真实 MySQL/Qdrant/Redis/Worker 与合成资料联合运行，不把替身验证等同真实模型质量验收。同步 `api.md`、`database.md`、`architecture.md`、`system-features-status.md`、Docker 文档、README 和 AGENTS。真实供应商测试待管理员配置及对应调用许可；业务库迁移仍独立确认备份/恢复。

### 业务库升级方案（2026-09-09 已授权并执行）

2026-09-09 只读确认业务库 Alembic 为 `20260705_03`。启动当前代码前需要顺序应用：

1. `20260902_01`：RAG 知识库/文档/块/作业/回答明细表，以及普通历史查询所需 `evaluation_tasks.task_type`。
2. `20260902_02`：块正文扩大为 MEDIUMTEXT。
3. `20260909_01`：全局 `embedding_config` 单例表，初始化为空配置，不创建云端供应商或密钥。

授权后先停止业务后端与 Worker 的写入入口（当前尚未运行），保留 MySQL/Redis/Qdrant。使用容器内 `mysqldump --single-transaction --no-tablespaces` 生成业务库备份，主机保存到 `backups/pre-v3-20260909.sql`，该目录加入本地 Git 排除。凭据仅从容器既有环境读取，不打印或写进脚本。检查导出退出码、文件非空及 SQL 结束标记，再执行 `alembic upgrade head`；升级后核对版本与表字段并启动普通业务栈（不启用 local-embedding profile）。建表和修改列可能持有元数据锁，选择无写入窗口。

失败时停止后端与 Worker，保留失败现场；不自动 downgrade 或恢复覆盖。经确认后在同一库导入上述完整备份，并使用升级前 Git 版本重建应用镜像；恢复将覆盖备份之后的写入，需要单独确认。未完成备份检查不得执行迁移；业务库操作与隔离测试授权分开。

- 2026-09-09：确认 `dev` 工作区起始干净，Docker Engine 29.7.2；现有 MySQL、Redis、Qdrant 在运行。
- 业务备份 `backups/pre-v3-20260909.sql`：49,583 字节，mysqldump 成功、结束标记通过，SHA-256 `5561ADC614887662400B7C31EB4A930ED94C2ACCCC3DE3236C7BD5E45BC5C29B`。备份目录已加入 `.git/info/exclude`，未提交。三项迁移成功，实查版本 `20260909_01`、task_type 字段和配置单例存在；未执行恢复演练。

### 恢复开发实际验收记录

- 管理员全局单例配置、兼容接口、字符切块、模型切换重建保护、按配置隔离及历史向量清理、React 页面与导航已实现。运行中保存、版本冲突、密钥保留/轮换/清除、普通用户拒绝管理均有测试。
- 后端完整镜像构建及 pip check 通过。首轮两个历史 API 断言遗漏 taskType，已修正；全量复验 `346 passed / 44 skipped`。最后补充旧集合删除隔离用例及运行配置小调整后，相关客户端/配置/服务测试 `56 passed`。跳过项包含需要真实 TEI/缓存、隔离库和外部 CLI 的用例，不等于通过。
- React `63 passed`（15 文件），TypeScript/Vite 构建通过。旧导航精确列表补入 Embedding 入口后复验通过。
- 独立 MySQL 迁移及权限/作业/索引/块正文/恢复/用量回归 `38 passed`。
- 真实 MySQL/Qdrant/Redis/Worker + 确定性 Embedding/候选/Judge HTTP 联合用例：两候选流式评分/私有历史/反馈/评论/删除保留快照、候选失败隔离 `2 passed`；TXT/MD/PDF/DOCX 上传索引、整库重建及删除 `1 passed`。此处使用兼容 API 和 3 维替身向量，不加载模型，不代表语义质量。
- 业务 backend/frontend 健康，rag-worker 运行，MySQL/Redis/Qdrant 保留；后端直连与前端代理 `/api/health` 均返回 ok。浏览器已看到真实登录页及用户名/密码/登录控件，未取得管理员登录态，未验证登录后的管理页面实际点击。
- 隔离测试服务停止以释放内存，测试卷保留。未启动本地 TEI、未配置业务云端 API Key、未调用收费模型。实际供应商接入/质量、完整浏览器交互、100,000 块/并发性能与恢复演练仍未验收，V3 整体不能标记最终验收完成。
- 新增 api 脚本档已通过配置解析，其组成命令与联合测试已执行；完整跨平台入口及真实 TEI 档须按相应环境另行核验。改动保留在 dev 工作区，未提交或推送。
...
实施细化（2026-09-03）：沿用现有深色导航、绿色主色和 Ant Design 表单，不重做品牌/普通评测布局，不新增前端依赖。管理页采用库列表＋当前文档工作区，分页、单文件上传、只读进度、显式重试和危险操作确认；切分参数编辑明确需要重建，轮询与切换均取消过期请求。RAG 工作台固定私有/三轮 Judge，在开始按钮附近展示外部 API 数据边界；各候选有自己的进度、改写和证据折叠面板，未知引用单独提示，不跨卡共享 S1。评分详情逐轮展开断言与原始结构化结果，费用以表格按阶段和币种展示，未知用量显示“已知小计”。

新页面所有事件、API 调度和状态转换放 `.ts` hooks；`.tsx` 只组装 UI。复用现有请求错误/鉴权和 NDJSON 批处理，补 AbortSignal 与 reader 清理；历史管理行为提取到 hook 后增加类型筛选和过期响应保护。RAG 历史超时标记使用 60 分钟（chat 保留 2 分钟），私有评论使用明确文案。测试编写接口 mock、纯状态和静态组件渲染；Docker 关闭时不在宿主机安装/运行 Node，也不把未运行的 Vitest/类型检查/视觉验收写成通过。
...
实施记录（2026-09-03）：新增两个 React 页面、类型化 API、知识库/RAG hooks、证据与联合评分组件；复用主题、Ant Design、Markdown、反馈与流式批处理，无新增依赖，Vue 不改。管理页覆盖分页、容量/格式检查、上传/下载、状态/进度、配置修改、重建/删除确认和失败重试。RAG 页固定私有、三轮空闲 Judge，保留思考开关与发送范围提示；停止/离开取消请求，切换上下文丢弃旧批次。历史行为提取到 `.ts`，增加类型筛选、RAG 60 分钟边界、预创建回答的运行阶段和私有评论文案。

新增测试代码覆盖真实请求体/信号、NDJSON 读取器清理、领域错误文案、阶段转换、强制私有/Judge、上传边界、历史超时和证据/私有评论静态渲染。当前未启动 Docker，没有执行 Vitest、TypeScript/Vite 构建、SSR 测试或浏览器交互；静态渲染用例也不能代替点击、键盘、移动端和请求竞争验收。提交前仅人工核对接口/类型/文档、执行 Git 差异检查；阶段 6 以“代码交付、验证延期”记录。

后续浏览器必验：切库时延迟旧响应不覆盖；切类型/任务时旧详情不覆盖；退出后无轮询/流式请求；两回答均含 S1 时分别展开自己的正文；删除/重建确认、失败重试、404 和上传 20 MB 边界；停止后刷新历史显示最终失败/成功及用量；键盘展开证据、弹窗滚动、320/768/1280px 布局。以上当前均未执行。
...
## 2026-09-09 评测一致性改进规格与计划

1. 普通与 RAG 共用既有 daily_user_token_usage 统计，不新建账本。修复管理员今日用量恒为零；管理员仍不限额。RAG 沿用改写、生成与评分已知 Token 汇总，未知用量不伪造，Embedding 文档索引不纳入对话额度。任务结束、页面进入与窗口重新聚焦刷新。不得重复入账，不修改业务历史记录。
2. 导航使用四个可展开一级分类：普通评测（工作台、历史任务、反馈统计）、RAG 评测（工作台、知识库）、Agent 评测（待开发）、系统设置（模型设置、用户额度、评分配置）。复用已有路由与权限；模型设置将普通大模型和 Embedding 合为两个页签，兼容旧地址。历史/反馈仍为跨类型共享页面，保留原筛选能力。
3. RAG、知识库、Embedding 采用现有 page-head、卡片、表单与 Token 面板的间距、字体、颜色规范，移动端与桌面一致。新增交互逻辑放 .ts，不扩展 Vue。
4. RAG 默认启用思考。前后端均防止评分模型与候选模型相同；同时检查重复配置的实际模型身份，避免不同配置 ID 绕过。前端候选变化应重选可用评分模型，无可用者禁止提交。
5. 验收：额度管理员/普通用户与共用账本回归、RAG 评分身份与思考默认测试、导航权限/分组/兼容路由测试、前端全量测试与构建；仅隔离测试库可写，不发送真实知识库文档进行验证。浏览器检查视登录态可用性记录。

### 2026-09-09 UI 调整补充

- 左侧导航按普通评测、RAG 评测、Agent 评测、系统设置四组折叠展示；Agent 当前仅显示待开发状态，系统设置仅管理员可见。
- 系统设置只保留一个模型设置入口，大模型与 Embedding 配置通过页签切换；`/embedding-config` 继续作为兼容地址，不再单独显示在导航中。
- Embedding 页面统一使用旧页面的 `page-head`、管理卡片、表单间距和颜色规范；知识库与 RAG 页面沿用同一套卡片和工具栏视觉类名。

### 本轮最终验收（2026-09-09）

- 业务库只读核对：当前用户 RAG 已入账 13,783 Token；修复后 TokenQuotaService 返回 usedTokens=13783、unlimited=true，无需回填或修改业务数据。
- 管理员显示用量回归先复现失败后通过；定向用量/API/阶段用量 14 项通过。
- 正确隔离 unit-runner 全量后端：350 passed、44 skipped。此前业务容器直接执行全量时 17 项因缺少仓库根脚本/文件挂载失败，改用隔离入口后通过。
- 前端全量 67 项通过，TypeScript 与 Vite 构建通过；另补模型页签和旧地址定位回归。
- 浏览器工具两次返回 Unable to load browser request-header policy，真实登录后点击、窗口聚焦刷新及视觉验收未执行；不将组件测试等同真实浏览器通过。
- 新增逻辑与 UI 已热更新；本轮没有业务数据写入、真实文档外发、提交或推送。普通/RAG 共用账本范围保持既有规则：普通回答 Token；RAG 改写/生成/评分已知 Token；文档索引 Embedding 不计入对话额度。

补充验收：模型设置组件 3 项定向测试通过（包含新旧入口定位）；标准 Git 差异检查通过，前端代理健康检查返回 ok。

### 导航布局回归修复（2026-09-09）
用户截图复现：details 中的链接为行内排版，父级 Grid 未作用于披露内容，造成换行与点击区域重叠。为二级菜单增加独立 nav-group-items 纵向 Grid 容器，链接明确占满单行，summary 保持原生鼠标/键盘折叠操作；不变更路由和权限。验收检查展开后纵向布局、折叠隐藏、激活入口和窄屏点击范围。

老大补充：保留原有按钮样式，仅改为多级嵌套。因此保持 .side-nav a 的字体、颜色、圆角、padding 和选中样式，仅给子容器设置纵向 Grid 与链接块布局。Chrome 实测截图确认菜单逐行且无重叠，系统设置可收起/展开，点击模型配置成功进入 /models 并显示模型列表。导航测试 7 项及 TypeScript/Vite 构建通过。移动端本轮未实测。

一级菜单尺寸调整：按老大要求，summary 字号从 11px 增至 16px，内边距改为 14px 12px，行高 1.5；保留原颜色、展开逻辑和二级菜单样式。验收检查一级菜单可读性及无重叠。

一级菜单按钮交互：按老大要求，保留 details/summary 原生鼠标与键盘行为，增加整行圆角、常态背景与边框、hover/按下反馈、展开高亮及焦点轮廓，配色复用侧栏；二级菜单不变。仅修改 CSS，不改路由或数据。

按钮交互验收：Chrome 截图确认四个一级菜单均有完整圆角背景与边框；点击系统设置后正常展开三个二级入口。Git 差异检查通过。

导航微调：一级按钮常态与展开状态背景透明，保留 hover/按下、边框与焦点反馈；反馈统计从普通评测子菜单移出，位于 Agent 与系统设置之间作为一级直达链接。保留原 /feedback 路由和角色权限，桌面与移动导航复用同一结构。

## RAG 公开评测与历史可见性（2026-09-09）

老大授权本轮功能实现：RAG 支持选择公开/私有（默认私有，显式选择后公开）；公开任务供所有登录用户查看完整回答、评分与任务内引用证据快照，知识库本体/原始文档下载不随任务公开。移除 RAG 页资料发送提醒及仅自己可见标签，按钮改为开始评测。

普通历史 /history 固定 taskType=chat，RAG 历史 /rag/history 固定 taskType=rag，独立路由和对应导航，复用历史组件。任务作者可在历史详情切换可见性；新增 PATCH /api/evaluation/tasks/{task_id}/visibility，仅接受 public/private，只允许作者操作，非作者统一404。不重跑任务、不改变用量、评分或知识库权限。已有任务不批量改写，无结构迁移；业务可见性修改由用户在UI主动提交，本轮自动验证仅使用隔离测试数据。

实施顺序：排查并移除 RAG 任务级强制私有约束，但保留资源所有权约束；增加作者更新接口与跨用户公开/私有权限测试；拆分历史入口与UI；检查反馈/评论/用量/运行中收尾不因切换可见性失效；前后端回归与构建。

实施与验收：上述功能已实现；隔离 unit-runner 后端 355 passed、44 skipped，覆盖公开完整回答/证据、改回私有、非作者拒绝修改、普通/RAG 分类，以及公开任务评分收尾与用量入账。前端全量 70 项通过，包含导航、公开请求/流状态保持与可见性 PATCH 契约；TypeScript/Vite 构建通过。前端代理 `/api/health` 返回 ok，Git 差异检查通过。浏览器工具持续报 request-header policy 加载失败，本轮实际点击和视觉验收未完成；不将自动测试等同浏览器验证。已同步 API、架构、数据库说明、功能状态、README 和 AGENTS；无业务任务可见性写入、无新迁移、无提交或推送。

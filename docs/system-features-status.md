...
状态：**部分实现（基础设施、私有知识库 API、索引代码；完整联调待验收）**。新增 TEI CPU/Qwen3-Embedding-0.6B、Qdrant、Redis、Celery Worker 配置和固定版本只读分词缓存；客户端支持双上限分批、向量校验、错误脱敏及归属/版本过滤。阶段 2 增加私有库创建/列表/修改/删除、文档上传/下载/列表/重试及整库重建作业；每库 100 份、每份 20 MB，管理员无私有内容读取豁免。详见 `v3-rag-spec-plan.md` 的阶段验收记录。

阶段 3 已实现四格式深层解析、来源映射、Qwen Token 切分、租约/冷却/有限重试、幂等索引与三存储清理。按老大最新决定，资源不足的真实模型全链路测试和本地部署延期，阶段代码可先交付；详细已执行证据与待补项见计划，不能称完整联调已通过。索引发布需核对 MySQL/Qdrant 数量，删除先阻止访问再清理，失败不会冒充已删除。逐模型 RAG 回答、引用快照写入、忠实度评分及 React 页面尚未实现，没有开放 `taskType=rag` 或导航入口。迁移 `20260902_01/02` 已在隔离测试库验证，业务库尚未应用；旧问题/回答/得分不改。Agent 工具评测和安全测试集尚待单独设计。
